...
int Procesador::cantidad_procesos() const{
    return número_procesos;
}
...
int Procesador::mejor_hueco(const Proceso& proc) const{
    map<int, set<int>>::const_iterator it = map_huecos.end();
    if (it != map_huecos.begin()){
        --it;
        if ((*it).first >= proc.consultar_memoria()) return (*it).first;
    }
    //if (it != map_huecos.end() and (*it).first >= proc.consultar_memoria()) return (*it).first;
    return -1;
}
